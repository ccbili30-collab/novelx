...
- 当前模块化维护验收头：`3a28cb9`。该哈希之后的状态必须重新核对，不能沿用本节的测试结论。
...
- 当前冻结头已运行完整 `npm test`：Unit 728/728、Integration 22/22、e2e-support 6/6，零跳过；生产 `npm run build` 通过，并验证 3 组 active Prompt publication。未运行安装包、升级/重开、真实 Provider 或新的 Live，因此这些边界仍不得宣称完成。
