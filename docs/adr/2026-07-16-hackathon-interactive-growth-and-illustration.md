...
实现收口（2026-07-16）：Schema 26 与上述持久化不变量已经落地；Review 本身不再驱动 `closed/blocked`，只有同事务封存并使 Cycle 进入 `evaluated` 的 durable outcome 才能驱动 Closure 状态。旧 `legacy_pre_v26` accepted Review 仅供历史读取，当前投影保持 `growing`。Repair 只有在绑定 Cycle 已原子提交 Change Set 后才能进入 `committed/no_progress`；`resolved` 还要求该输出 checkpoint 上的新 Closure revision 获得后续独立 accepted outcome。当前公开 capability/strategy 已升为 v4。`closure_evaluation` 已接入 Worker/Main/Domain：Main 固定 Profile、revision、rule revision、checkpoint 与同一 Retrieval Receipt，把 evaluator 选中的目标版本强制纳入该 Receipt；Steward 与 Checker 使用同一 Run 中角色匹配的不同 invocation，并以其终态输出 hash 持久化严格 submission、Review 与原子 outcome。完整结果进入 `evaluated`，部分持久化或结果不明进入 `reconciliation_required`，不伪造 accepted。Coordinator 现会在初始内容前沿结束后创建默认 mixed Profile 与独立 evaluation Cycle；`repairs_required` 只选择一个 blocking/major finding，Repair Run 只取得该 finding 的固定证据并只允许一个 Change Set，提交后创建新 Closure revision 与下一 evaluation Cycle。Repair 提案现在还必须在 Main 根据该 finding 的 Receipt 证据解析出的资源、文档、断言或关系边界内；已有文档、断言、关系和约束档案必须以 pinned checkpoint 的真实 owner/scope/identity 回查，不能靠模型提交的 scope 换绑，缺失关系也只在两个端点均由 finding 直接选中时允许新建。计划态恢复会先校验 durable lineage，缺失时失败关闭，已 stalled 时不会重启 Worker。无进展计数只统计同一规则修订下连续未进展尝试；若复检已消除原 fingerprint 而出现另一 finding，则前一 repair 记为进展并为新 finding 建立独立 lineage。跨组件回归已覆盖 `repairs_required → repair committed → revision 2 recheck → accepted`、替换 finding 继续返工、缺失/停滞 lineage 恢复失败关闭，以及相同 finding 复检后 `no_progress/stalled` 且不再启动 Worker。Longform 的纯编译边界已经固定为“稳定 outline 约束文档 + 有来源的 OC 个人故事绑定”和“每节一个稳定 prose 文档”；创作内容来自模型，编译器只生成结构权威与拓扑。Runtime 分段编排与真实 Provider 验收仍未完成，不能据此宣称内容已闭环。

Longform 补充边界（2026-07-16）：个人长篇不得直接复用主线 story 作为字数容器，否则主线 prose 会被错误计入 OC 的 10,000 字闭环。编译器必须创建主线 story 下独立的 personal-story `volume`，为该 volume 建立 `uses_world`/`uses_oc`，再写入稳定 outline 与逐节 prose；Closure 的个人故事绑定只指向该 volume。

最新确定性验收覆盖合同、状态机、仓储、评估 Run、Coordinator 与 Supervisor 的 7 个 Vitest 文件、164/164 通过，`npm run typecheck`、`npm run verify:prompt-publication` 与 `git diff --check` 通过；本批未使用 Provider（模型服务）、Electron、生产构建或全量测试，因此只证明 Closure evaluation、自动调度、受信任 repair、accepted、替换 finding 进展与 stalled 的确定性闭环，不证明 Longform（长文）或 Live（真实运行）。
