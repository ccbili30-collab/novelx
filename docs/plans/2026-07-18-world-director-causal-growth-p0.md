...
**Task 12 evidence (2026-07-18):** the generic editorial specialist worker accepts only the nine fixed capabilities whose registered terminal tool is `submit_specialist_candidate`; Director, Graph Curator, Writer, Checker, GM and Decomposer fail closed instead of being routed through the wrong contract. Each invocation binds one profile, active published Prompt identity/content hash, pinned Work Order packet hash, exact evidence content hashes, acceptance facets and Harness-assigned Artifact slots. The Provider sees exactly one forced terminal tool. A ready submission returns strict `candidate + text/markdown artifacts`, with Artifact refs, evidence refs and facets checked against the prepared packet; `needs_more_evidence` requires zero Artifact and emits a separate evidence-requested terminal event with no Domain mutation path. Missing Provider configuration, candidate/unpublished Prompt, Prompt/packet/evidence tampering, arbitrary protocol tools, unsupported capability, forged refs/facets, duplicate accepted submission and cancellation all fail closed with content-free events. Four targeted runtime, registry, contract and runtime-profile files passed 23/23 tests with zero skips; `npm run typecheck`, `npm run build` and `git diff --check` passed. Tests used a fake RuntimeAdapter and synthetic active Prompt identity; no real Provider, E2E or full suite was run. Task 13 real Prompt assets/publication remain unimplemented, so the new editorial specialists are not yet production-live.
...
**Task 13 evidence (2026-07-18):** one versioned shared editorial baseline plus eleven role-specific Markdown assets now compose the fixed World Director, eight author/architect roles, Graph Curator and Visual Director candidates. Every composed asset binds the Task 11 Prompt ID/version and terminal tool, treats packet evidence as untrusted data, forbids role/scope/tool/Provider/Canon authority expansion, requires source-bound claims or explicit evidence requests, suppresses raw reasoning and states that actual Provider/model/Prompt/profile identity is Harness-audited. `editorialPromptRegistry` pins a distinct SHA-256 for each composed UTF-8 asset, rejects identity/hash/set drift and exposes a static contract lint explicitly classified `not-publication-evidence`. All eleven entries remain `candidate` with null publication evidence; fake activation and active loading fail closed. Four targeted Prompt, registry, Runtime and Task 4 contract files passed 24/24 tests with zero skips; `npm run typecheck` and `git diff --check` passed. No real Provider, publication evaluation, build, E2E or full suite was run. World Director and Graph Curator production runtimes do not yet exist, so real publication is intentionally deferred rather than simulated.
...
**Task 14 evidence (2026-07-18):** the pure Main packet compiler accepts only strict Creator Lens safe projections pinned to one branch/checkpoint/rule revision and emits a deterministic hashed World Director packet. It includes all user rules with verified content hashes, the code-owned editorial charter and fixed capability roster, bounded Closure matrix, sourced causal frontier, recent committed Change Set summaries/output kinds, unresolved Checker findings, upstream node-maturity projections, source-version-bound graph summaries and aggregate image queue state. Every expandable section has a stable order, count budget and omission receipt; truncated graph summaries also mark the packet incomplete. User rules are never silently sliced, and an oversized final packet fails closed. Checkpoint drift, rule tampering, duplicate identities, Player Lens, credential-like content and extra raw Prompt/database/hidden-player/unrelated-prose fields are rejected. Three targeted compiler, Prompt and capability files passed 14/14 tests with zero skips; `npm run typecheck` and `git diff --check` passed. No Provider, database mutation, build, E2E or full suite was run. The compiler trusts only caller-supplied Creator-safe projections; repository gathering remains scheduler/Main integration work, and Task 21 still owns the node-maturity algorithm.
...
**Task 18 evidence (2026-07-18):** the dedicated Agent Worker Graph Curator route accepts one strict checkpoint/Work Order packet with authorized resource scope, existing assertion refs and bounded evidence content. It verifies Provider presence, published Prompt identity/content hash, fixed capability profile, packet/evidence hashes and credential-like content before Provider execution, then exposes exactly one forced `submit_graph_curator_candidate` tool. A ready result must bind every assertion and causal link to supplied `@evidence` code-point ranges whose exact slice SHA-256 is recomputed; assertion subjects must remain inside authorized resource/assertion/evidence/local identities, and causal endpoints must be candidate-local or explicitly supplied assertion identities. Self-edges, empty mechanisms/conditions/sources, forged refs/ranges/hashes/scopes/endpoints, extra output fields and duplicate submissions fail closed. The alternative `needs_more_evidence` terminal state contains bounded queries and supplied evidence refs but no graph candidate; the unsupported-correlation test uses this path rather than fabricating an edge. The Worker process routes the command and emits only safe receipt/error projections. Five targeted Graph Curator/registry/editorial/specialist/Worker suites passed 53/53 tests with zero skips; `npm run typecheck`, `npm run build` and `git diff --check` passed. Tests used an in-memory Runtime adapter and synthetic active Prompt; no real Provider ran, the actual Graph Curator Prompt remains candidate/unpublished, and no Domain graph mutation occurred. Exact citation proves source binding, not semantic causal truth; Task 19's deterministic candidate policy must still reject cited-but-unsupported mechanisms before Change Set compilation.
...
**Task 19 evidence (2026-07-18):** a Main-owned deterministic compiler now accepts only ready specialist and Graph Curator submissions plus Harness-authorized resource, artifact, evidence, existing-assertion and causal-support maps. It generates every `resource.put`, `document.put`, `assertion.put` and `causal_relation.put` item identity, Domain identity, parent/ownership mapping, local/existing endpoint mapping, dependency edge and same-Change-Set document evidence token from the attempt and authorized refs; the proposal uses the existing strict `propose_change_set` contract and canonical audit hash. The policy requires exact specialist artifact and causal-support sets, recomputes same-batch Artifact whole-content and Unicode code-point slice hashes, rejects unsupported or unknown causality, unresolved endpoints, invalid parent ordering/scope, duplicate identities, forged fields and source drift before any persistence. The existing internal Graph Curator contract was minimally aligned with the already-authoritative Domain causal policy by adding `relationKind` and `polarityStrengthSummary`; no public IPC or SQLite schema changed. A real SQLite/Gateway/ChangeSetService test commits one document, two sourced assertions and one causal relation atomically, while failure tests assert zero database writes before compilation succeeds. Six related suites passed 75/75 with zero skips; `npm run typecheck`, `npm run build` and `git diff --check` passed. No phase-registry change was needed because compilation is a Main handler seam and did not add a Steward state-machine branch. No real Provider, Renderer, E2E or full suite ran; this is not a complete World Director or world-package Live.
...
**Task 20 evidence (2026-07-18):** the strict internal World Fragment contract now requires one world plus at least twelve source-bound skeleton entities: three macro regions, four polity/civilization groups assigned across at least two declared regions, and explicit mountain, sea, river, transport-network and resource-distribution nodes. It also requires four eras, three historical turning points and four causal mechanisms whose assertion endpoints exist, whose document sources exist and whose system sets contain at least two distinct allowlisted systems. The Harness persists entity roles, eras and turning points as fixed-predicate sourced assertions and compiles every mechanism as a real `causal_relation.put` with generated identities, assertion dependencies and same-Change-Set document sources; titles and keyword matching are not completion authority. The separate world-scale Closure profile reports eleven exact satisfied/missing facets, rejects unsourced entries, invalid regional ownership and non-cross-system mechanisms, and does not treat missing evidence as partial success. A package-skeleton fixture committed through real SQLite, Gateway and ChangeSetService using existing Domain rules; every old small-world fixture was upgraded rather than accepted through a compatibility path. Six related suites passed 109/109 with zero skips; `npm run typecheck`, `npm run build` and `git diff --check` passed. No real Provider, Renderer E2E, full suite or world-package export ran; this proves the structural floor, not Task 21 node depth or the final Live loop.
...
**Task 21 evidence (2026-07-18):** one pure Domain policy now owns the executable coverage dimensions for OC (14), nation (16), organization (12), geography (12), species (14), civilization (16), story (8) and world (8). Nation and civilization intentionally reuse one authoritative dimension set instead of duplicating business rules. Evaluation accepts only source-bound, unique, profile-authorized dimension coverage; it returns every covered/missing criterion, full-depth status and the exact blocking gaps. Core and major nodes require all dimensions. Supporting and background nodes expose the same gaps without inventing an unapproved percentage threshold or presenting zero coverage as full depth. A user promotion is monotonic; promoting to major/core immediately re-evaluates the existing evidence against the full profile, while same-tier or downward transitions fail closed. Cross-profile dimensions, duplicate coverage/evidence and extra model-supplied authority fields are rejected. The targeted pure-policy suite passed 7/7 with zero skips; `npm run typecheck` and `git diff --check` passed. No build, SQLite, Provider, E2E or full suite ran. Task 26 still owns integration into global convergence, so these profiles alone do not complete a node or world package.
...
**Task 22 evidence (2026-07-18):** a strict Harness-only causal selector accepts three to seven projected gaps, each with an allowlisted causal-gap kind, affected node refs, pinned evidence refs, missing-link count, downstream-block count, importance tier and a bounded decision class. Code ranks importance before downstream impact and missing links, then uses stable gap identity as a tie-breaker. A canonical fingerprint over gap kind, facet, sorted nodes and sorted evidence deduplicates question rewordings; prior fingerprints are excluded, and an unchanged exhausted set returns `no_progress` instead of looping. Only mutually exclusive Canon, conflicting user rules or an irreversible world premise may produce `ask_user`; all other selected gaps remain autonomous. The Inquiry phase compiler binds the complete candidate set, selected question, evidence and decision back to the model Brief, so model priority cannot override Harness selection. A Closure regression proves that a 3/4 cross-system mechanism gap outranks major/supporting gaps. The separate Worker→Main protocol does not yet carry affected-node metadata and was deliberately left unchanged rather than creating a one-sided fake integration; the structured selection remains available at the phase seam for the Task 26 convergence caller. Eight related Inquiry, Closure and Worker suites passed 99/99 with zero skips; `npm run typecheck`, `npm run build` and `git diff --check` passed. No SQLite persistence, real Provider, E2E or full suite ran.
...
**Task 28 evidence (2026-07-18):** a capability-local safe diagnostic catalog now maps an exact allowlist of Director packet/planning, Work Order lifecycle, specialist candidate protocol, graph-causal evidence, review, Provider, Domain, persistence and reconciliation source codes into bounded ownership families. Unknown values—including forged `PROVIDER_*` or `DOMAIN_CAUSAL_*` prefixes, objects carrying a safe-looking `code`, and secret-bearing strings—collapse to `WORK_ORDER_STATE_UNCLASSIFIED_FAILURE`; raw exceptions, Provider bodies, Prompt content and arbitrary dynamic codes are never appended to `safe_diagnostic_events`. The existing envelope vocabulary remains unchanged: editorial plan/work state use Growth phase boundaries, specialist protocol uses Worker schema, graph/Domain uses Domain policy, reviews use phase correction, and Provider/persistence/reconciliation retain their existing owners. Work Order candidate/recheck failures, outcome-unknown commits and scheduler-owned dependency failures append diagnostics only after durable editorial state, replay once per terminal class and never replace authoritative state if diagnostic persistence fails. Four focused diagnostic/scheduler/contract/repository suites passed 42/42 with zero skips; `npm run typecheck` and `git diff --check` passed. Tests use real SQLite with injected editorial failures, not a real Provider; no Electron E2E, full suite, build, package or world-package export ran. The envelope has no editorial-round operation kind, so these records use the existing `tool_call` operation identity keyed by Work Order and are not yet a new public Renderer diagnostic feed; Task 29 still owns restart/reconciliation truth across all editorial boundaries.
...
| [x] | 18 | Specialist prose does not automatically become trustworthy graph structure. | Graph Curator returns sourced assertions and causal candidates with no Domain side effect. | Source-span, mechanism, condition, uncertainty and unsupported-inference tests. |
| [x] | 19 | Curator output, Checker findings and specialist content must converge before writing. | A deterministic compiler produces one authorized Change Set or one precise blocker. | Atomic compiled-output integration tests and zero-write rejection tests. |
