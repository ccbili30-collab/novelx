...
| Automatic Agent allocation | Durable Assignment commands, allocation-revision child Run pins, immutable ChildRunSpec（子运行规格）, cross-language canonical identity hashes and a read-only structural recovery/quarantine barrier are live; a missing child with a valid specification is classified only as a provision candidate | Stable Saga（事务链）operation IDs, actual child Run provisioning, Provider-bind operational recovery, bounded concurrency, cancellation propagation, synthesis, cost controls and real multi-Agent cross-process acceptance |
