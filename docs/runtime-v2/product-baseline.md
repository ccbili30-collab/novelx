...
| Automatic Agent allocation | Durable Assignment commands, immutable ChildRunSpec（子运行规格）, stable Saga（事务链）identities, structural recovery, evidence fingerprints, an append-only operational recovery ledger, cross-journal mutation clock, fresh-rescan fenced Claim coordinator, bounded server-time leases and persisted local/projection-only execution lifecycle are live; exact Provider binding still records a candidate and sends no model request | Exclusive supervisor ownership, safe pre-execution Claim transfer/stale handling, actual child Run provisioning, evidence-first Provider continuation, verified Tool manifests, bounded concurrency, cancellation propagation, synthesis, cost controls and real multi-Agent crash/restart acceptance |
