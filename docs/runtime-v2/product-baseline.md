...
| Automatic Agent allocation | Durable Assignment commands, immutable ChildRunSpec（子运行规格）, stable Saga（事务链）identities, structural recovery, evidence fingerprints, append-only recovery ledger, cross-journal mutation clock, Windows workspace Runtime lock, fresh-rescan fenced Claim coordinator, safe expired/unstarted Claim transfer, bounded server-time leases and persisted local/projection-only execution lifecycle are live; exact Provider binding still sends no model request | Pre-execution stale terminalization, automatic exclusive Supervisor scheduling, actual child Run provisioning, evidence-first Provider continuation, verified Tool manifests, bounded concurrency, cancellation propagation, synthesis, cost controls and real multi-Agent crash/restart acceptance |
