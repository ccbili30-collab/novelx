...
用户已决定采用双头脑路线：`work\main` 的 Main 头脑负责长期产品、Rust Runtime V2、完整 Harness 和赛后主线；本工作树的 Worktree 头脑负责 10 天黑客松的战术架构、演示路线、任务拆分、文件所有权、验收标准和结果审查。两边都可以把具体编码交给各自的有界执行会话。

Worktree 头脑可以在既定黑客松产品闭环和冻结边界内决定实现顺序、局部适配与演示 P0，但不能改写完整产品 Source of Truth、长期 Runtime 架构、公开协议、数据模型、权限边界或赛后合并策略。遇到这些问题时，必须提交给产品负责人和 Main 头脑共同决策。

每个执行任务包必须包含目标、允许文件、禁止文件、验收命令、完成条件和停止条件；执行会话不得自行进入下一阶段。
